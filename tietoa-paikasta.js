    ...
    if (shareBtn) {
        ...
        if (!sharePanel) {
            ...
            sharePanel.innerHTML = `
                <p style="margin:0 0 0.5rem; font-size:0.8rem; font-weight:700; color:#64748b; text-transform:uppercase; letter-spacing:0.5px;">Jaa kohde</p>
        ...
        document.addEventListener('click', (e) => {
            ...
            if (e.target.closest('#share-qr-copy-btn')) {
                ...
                fetch(qrImg.src).then(res => res.blob()).then(blob => {
                    ...
                    a.download = `laukaainfo-qr-${safeName}.png`;
                    a.click();
                    URL.revokeObjectURL(a.href);
                });
            }
            if (!e.target.closest('#share-dropdown-panel') && !e.target.closest('#share-place-btn')) {
...
async function loadMixonetContentForPlace(placeData) {
    ...
    try {
        ...
        (directProjectsRes.data || []).forEach(p => combinedProjectsMap.set(String(p.id), p));
        (customProjectsRes.data || []).forEach(p => combinedProjectsMap.set(String(p.id), p));
        (relProjectsRes.data || []).forEach(p => combinedProjectsMap.set(String(p.id), p));
        const allProjects = Array.from(combinedProjectsMap.values());

        const combinedIdeasMap = new Map();
        (relIdeasRes.data || []).forEach(i => combinedIdeasMap.set(String(i.id), i));
        ...
        // 5. Renderöi projektit (ensisijaisesti pääosioon #mixonet-projects-section, ettei tule tuplana)
