        function isMatch(c, opt, context, subContextsReq, noCateringSelected) {
            ...
            // TAKSONOMIA-TÄSMÄYS (UUSI)
            if (typeof taxonomyData !== 'undefined' && taxonomyData && !isProfiledMatch && !opt.profilointi_filter) {
            ...
            // Broaden photography/videography match
            if (!isProfiledMatch && opt.tags) {
                const isPhotoSearch = opt.tags.some(t => typeof t === 'string' && t.includes('valokuva'));
                const isVideoSearch = opt.tags.some(t => typeof t === 'string' && (t.includes('video') || t === 'videotuotanto'));
                const isFloristSearch = opt.tags.some(t => typeof t === 'string' && t.includes('kukka'));
            ...
            // Digitointi ja juhlatila tags
            if (!isProfiledMatch && opt.tags) {
                if (opt.tags.includes('digitointi')) {
                    const catData = getCategoryData(c, context);
                    const hasDigitizationFeatures = catData && Array.isArray(catData.digitization_features) && catData.digitization_features.length > 0;
                    
                    if (hasDigitizationFeatures && fitsScore > 0) {
                        isProfiledMatch = true;
                    } else {
                        const companyContent = (
                            (c.tags || '') + ' ' + 
                        ...
                        if (companyContent.includes('digitointi')) {
                            isProfiledMatch = true;
                        }
                    }
                }
                if (opt.tags.includes('juhlatila')) {
