...
=======
      // ── Supabase live-haku ──
      const POSTS_SUPABASE_URL = 'https://usswojtlvrnqtzwnffpg.supabase.co';
      const POSTS_SUPABASE_KEY = 'sb_publishable_SI9jkzJCyrxXQebhuoQGqQ_LN9wH8hl';
      
      const supabasePromise = fetch(
        `${POSTS_SUPABASE_URL}/rest/v1/posts?select=*&order=created_at.desc&limit=20`,
        {
          headers: {
            'apikey': POSTS_SUPABASE_KEY,
            'Authorization': `Bearer ${POSTS_SUPABASE_KEY}`,
            'Accept': 'application/json'
          }
        }
      ).then(r => r.ok ? r.json() : []).then(posts => {
        return posts.map(post => ({
          id: 'post_' + post.id,
          business_id: post.business_id,
          type: post.type === 'event' ? 'event' : (post.type === 'offer' ? 'offer' : 'notice'),
          title: post.title || '',
          ...
          is_shorts: post.is_shorts || false,
          ...
          show_contact: post.show_contact || false
        }));
      }).catch(() => []);

>>>>>>> 086a5382df8f2ecf5a9717111a67cf09af2d2b6d
...
=======
      Promise.all([fetchPromise, pikkuPromise, supabasePromise, minDelay])
        .then(([res, pikkuRes, supaRes]) => {
          let apiData = Array.isArray(res) ? res : (res.data || []);
          let data = [...supaRes, ...apiData];
          
          // Sort by publish_at descending
          data.sort((a, b) => new Date(b.publish_at || 0) - new Date(a.publish_at || 0));
